...
export function Legend() {
  return (
    <div className="flex flex-wrap gap-3">
      {/* Half vacation: blue top-left triangle */}
      <div className="flex items-center gap-1.5">
        <div className="relative w-4 h-4 rounded overflow-hidden">
          <div className="absolute inset-0 bg-white dark:bg-gray-800" />
          <div className="absolute inset-0 bg-blue-500" style={{ clipPath: 'polygon(0 0, 100% 0, 0 100%)' }} />
        </div>
        <span className="text-xs text-gray-500 dark:text-gray-400">Half vacation</span>
      </div>
      {SOLID_ENTRIES.map(({ label, className }) => (
